...
namespace {
  ...
  // generate_lion_moves generates the special double-move for lion-type pieces.
  // This adds igui (stay-capture) and mid-capture+continue moves on top of
  // the normal moves already generated by generate_moves. The lion's regular
  // one-step and two-step leap moves are handled by generate_moves via the
  // piece's normal attack pattern (BETZA "free 2" = KAD).
  template<Color Us, GenType Type>
  ExtMove* generate_lion_moves(const Position& pos, ExtMove* moveList, PieceType Pt, Bitboard target) {
    constexpr Color Them = ~Us;
    Bitboard bb = pos.pieces(Us, Pt);
    ...
    while (bb) {
        ...
        // Enumerate all adjacent squares as the intermediate (mid) step.
        // Only enemy-occupied mids produce LION_MOVE; empty mids are handled
        // by normal move generation as two-step leaps.
        Bitboard midSquares = pos.attacks_from(Us, KING, from) & pos.pieces(Them);

        Bitboard mids = midSquares;
        while (mids) {
            Square mid = pop_lsb(mids);
            PieceType midPt = type_of(pos.piece_on(mid));

            if (Type == CAPTURES || Type == NON_EVASIONS || Type == EVASIONS) {
                // Igui: capture at mid, return to from (from == to).
                // Always valid as a capture; not filtered by target since 'from' is
                // a friendly square and would never appear in the normal target set.
                *moveList++ = make_lion(from, from, mid, midPt);

                // Continue: capture at mid then step to any adjacent square of mid
                Bitboard tos = pos.attacks_from(Us, KING, mid) & ~pos.pieces(Us) & target;
                tos &= ~square_bb(mid);  // can't stay at mid (already captured it)
                tos &= ~square_bb(from); // igui (stay at from) already emitted above
                while (tos) {
                    Square to = pop_lsb(tos);
                    *moveList++ = make_lion(from, to, mid, midPt);
                }
            }
            else if (Type == QUIETS || Type == QUIET_CHECKS) {
                // Quiet destinations after capturing at mid (no second capture)
                Bitboard tos = pos.attacks_from(Us, KING, mid) & ~pos.pieces() & target;
                tos &= ~square_bb(from); // igui is a capture, not a quiet
                while (tos) {
                    Square to = pop_lsb(tos);
                    *moveList++ = make_lion(from, to, mid, midPt);
                }
  ...
  // generate_lion_dog_moves generates LION_DOG_MOVE captures for Lion Dog type pieces.
  // Quiet moves (leaps to distance 1-3 along one of the 8 king-directions) are
  // handled by the piece's Betza pattern (KDAHG). This generates capture-and-continue
  // sequences that are strictly straight-line (one fixed direction per move):
  //   - Igui: capture s1, return to from
  //   - Capture s1 + land at s2 (one step further along direction)
  //   - Two-cap igui: capture s1 and s2, land back at s1
  //   - Two-cap forward: capture s1 and s2, land at s3
  //   - Cap s1 + quiet s2 + land at s3 (three steps, one mid-cap)
  //   - Quiet s1 + cap s2 + land at s3 (three steps, one mid-cap at distance 2)
  // All steps use the same Direction D; no direction change within a single move.
  template<Color Us, GenType Type>
  ExtMove* generate_lion_dog_moves(const Position& pos, ExtMove* moveList, PieceType Pt, Bitboard target) {
    constexpr Color Them = ~Us;
    static constexpr Direction DIRS[8] = {
        NORTH, NORTH_EAST, EAST, SOUTH_EAST,
        SOUTH, SOUTH_WEST, WEST, NORTH_WEST
    };
    const Bitboard boardBB = pos.board_bb();
    Bitboard bb = pos.pieces(Us, Pt);
    ...
    while (bb) {
        ...
        for (Direction D : DIRS) {
            Square s1 = from + D;
            if (!is_ok(s1) || !(boardBB & square_bb(s1))) continue;

            if (pos.pieces(Them) & s1) {
                // --- Enemy at s1 ---
                PieceType s1Pt = type_of(pos.piece_on(s1));

                // Igui: capture s1, return to from
                if (Type == CAPTURES || Type == NON_EVASIONS || Type == EVASIONS)
                    *moveList++ = make_lion_dog(from, from, s1, s1Pt);

                Square s2 = s1 + D;
                bool s2ok = is_ok(s2) && (boardBB & square_bb(s2));

                // Capture s1, land at s2
                if (s2ok && !(pos.pieces(Us) & s2) && (target & s2))
                    *moveList++ = make_lion_dog(from, s2, s1, s1Pt);

                if (s2ok) {
                    if (pos.pieces(Them) & s2) {
                        PieceType s2Pt = type_of(pos.piece_on(s2));
                        // Two-cap igui: capture s1 and s2, land back at s1
                        if (Type == CAPTURES || Type == NON_EVASIONS || Type == EVASIONS)
                            *moveList++ = make_lion_dog(from, s1, s1, s1Pt, s2, s2Pt);
                        // Two-cap forward: capture s1 and s2, land at s3
                        Square s3 = s2 + D;
                        if (is_ok(s3) && (boardBB & square_bb(s3)) && !(pos.pieces(Us) & s3) && (target & s3))
                            *moveList++ = make_lion_dog(from, s3, s1, s1Pt, s2, s2Pt);
                    }
                    else if (!(pos.pieces() & s2)) {
                        // Empty s2: capture s1, pass through s2, land at s3
                        Square s3 = s2 + D;
                        if (is_ok(s3) && (boardBB & square_bb(s3)) && !(pos.pieces(Us) & s3) && (target & s3))
                            *moveList++ = make_lion_dog(from, s3, s1, s1Pt);
                    }
                }
            }
            else if (!(pos.pieces() & s1)) {
                // --- Empty s1: look ahead to s2 ---
                Square s2 = s1 + D;
                if (!is_ok(s2) || !(boardBB & square_bb(s2))) continue;
                if (pos.pieces(Them) & s2) {
                    PieceType s2Pt = type_of(pos.piece_on(s2));
                    // Quiet s1, capture s2, land at s3
                    Square s3 = s2 + D;
                    if (is_ok(s3) && (boardBB & square_bb(s3)) && !(pos.pieces(Us) & s3) && (target & s3))
                        *moveList++ = make_lion_dog(from, s3, s2, s2Pt);
                }
            }
        }
    }
  ...
  // generate_hook_moves: Hook Mover (bent orthogonal rider).
  // Slides any distance in one orthogonal direction through empty squares, then
  // turns 90° (either perpendicular) and slides until blocked/capture.
  // Both perpendicular directions must be tried: a blocker on one side doesn't
  // prevent hooked access from the other side via a different entry column/row.
  // A visited bitboard deduplicates squares reachable via two bend paths
  // (e.g. N→E from d5 and E→N from e4 both reach e5 on an empty board).
  // Straight Rook slides are handled by the piece's Betza "R" pattern.
  template<Color Us, GenType Type>
  ExtMove* generate_hook_moves(const Position& pos, ExtMove* moveList, PieceType Pt, Bitboard target) {
    constexpr Color Them = ~Us;
    static constexpr Direction DIRS[4] = {NORTH, EAST, SOUTH, WEST};
    static constexpr Direction PERPS[4][2] = {
        {EAST, WEST},    // N perps
        {NORTH, SOUTH},  // E perps
        {EAST, WEST},    // S perps
        {NORTH, SOUTH}   // W perps
    ...
    while (bb) {
        ...
        for (int di = 0; di < 4; di++) {
            Square corner = from;
            for (;;) {
                corner = corner + DIRS[di];
                if (!is_ok(corner) || !(boardBB & square_bb(corner))) break;
                if (pos.pieces() & corner) break;  // blocked: can't hook past a piece
                for (int pi = 0; pi < 2; pi++) {
                    Square to = corner;
                    for (;;) {
                        to = to + PERPS[di][pi];
                        if (!is_ok(to) || !(boardBB & square_bb(to))) break;
                        if (pos.pieces(Us) & to) break;
                        if (!(visited & square_bb(to))) {
                            if (target & to) *moveList++ = make_move(from, to);
                            visited |= square_bb(to);
                        }
                        if (pos.pieces(Them) & to) break;
                    }
                }
            }
        }
  ...
  // generate_goblin_hook_moves: Long Nosed Goblin / Goblin Hook (bent diagonal rider).
  // Same as Hook Mover but with diagonal initial and perpendicular diagonal turns.
  // Both perpendicular diagonals tried; visited bitboard deduplicates same destination
  // reachable via two different bend paths.
  // Straight Bishop slides are handled by the piece's Betza "B" pattern.
  template<Color Us, GenType Type>
  ExtMove* generate_goblin_hook_moves(const Position& pos, ExtMove* moveList, PieceType Pt, Bitboard target) {
    constexpr Color Them = ~Us;
    static constexpr Direction DIRS[4] = {NORTH_EAST, NORTH_WEST, SOUTH_EAST, SOUTH_WEST};
    static constexpr Direction PERPS[4][2] = {
        {NORTH_WEST, SOUTH_EAST},  // NE perps
        {NORTH_EAST, SOUTH_WEST},  // NW perps
        {NORTH_EAST, SOUTH_WEST},  // SE perps
        {NORTH_WEST, SOUTH_EAST}   // SW perps
    };
    const Bitboard boardBB = pos.board_bb();
    Bitboard bb = pos.pieces(Us, Pt);
    while (bb) {
        ...
        for (int di = 0; di < 4; di++) {
            ...
            for (;;) {
                corner = corner + DIRS[di];
                if (!is_ok(corner) || !(boardBB & square_bb(corner))) break;
                if (pos.pieces() & corner) break;  // blocked
                for (int pi = 0; pi < 2; pi++) {
                    Square to = corner;
                    for (;;) {
                        to = to + PERPS[di][pi];
  ...
  ExtMove* generate_all(const Position& pos, ExtMove* moveList) {
    ...
    if (Type != EVASIONS || !more_than_one(pos.checkers() & ~pos.non_sliding_riders()))
    {
        ...
        // Lion double-move: igui and mid-capture+continue (on top of normal piece moves)
        if (pos.lion_move_types())
            for (PieceSet ps = pos.piece_types() & pos.lion_move_types() & ~(piece_set(PAWN) | KING); ps;)
                moveList = generate_lion_moves<Us, Type>(pos, moveList, pop_lsb(ps), target);
        // Lion Dog triple-move: igui and up to 2 mid-captures + final destination
        if (pos.lion_dog_move_types())
            for (PieceSet ps = pos.piece_types() & pos.lion_dog_move_types() & ~(piece_set(PAWN) | KING); ps;)
                moveList = generate_lion_dog_moves<Us, Type>(pos, moveList, pop_lsb(ps), target);
        // Hook Mover: orthogonal bent rider (slide orth → turn 90° → slide orth)
        if (pos.hook_mover_types())
            for (PieceSet ps = pos.piece_types() & pos.hook_mover_types() & ~(piece_set(PAWN) | KING); ps;)
                moveList = generate_hook_moves<Us, Type>(pos, moveList, pop_lsb(ps), target);
        // Goblin Hook (Long Nosed Goblin): diagonal bent rider (slide diag → turn 90° → slide diag)
        if (pos.goblin_hook_types())
            for (PieceSet ps = pos.piece_types() & pos.goblin_hook_types() & ~(piece_set(PAWN) | KING); ps;)
                moveList = generate_goblin_hook_moves<Us, Type>(pos, moveList, pop_lsb(ps), target);
        // generate drops
